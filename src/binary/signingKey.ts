/**
 * Anthropic's PGP public key used to sign Claude Code release manifests.
 *
 * Identity:    Anthropic Claude Code Release Signing <security@anthropic.com>
 * Algorithm:   RSA 4096
 * Fingerprint: 31DD DE24 DDFA B679 F42D 7BD2 BAA9 29FF 1A7E CACE
 * Source:      https://downloads.claude.ai/keys/claude-code.asc
 *
 * Embedded in source by design — fetching at install time would be
 * trivially spoofable by the same attacker who could substitute the
 * manifest. To re-verify the bytes below match upstream:
 *
 *     curl -sL https://downloads.claude.ai/keys/claude-code.asc | gpg --show-keys
 *
 * Should compare to the fingerprint above. If Anthropic rotates this key,
 * the plugin must ship an update with the new value here.
 */
export const CLAUDE_CODE_PUBLIC_KEY_ASC = `-----BEGIN PGP PUBLIC KEY BLOCK-----


9n3CmZUmbynZ+E/QWNENpW/pDNZdWFy43PASmML5FHu4m9Sn
=oqMI
-----END PGP PUBLIC KEY BLOCK-----
`;
